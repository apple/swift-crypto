        ...
        fileprivate final class Backing {
            ...
            init() {
                ...
                self.seed = withUnsafeTemporaryAllocation(
                    of: UInt8.self,
                    capacity: MLKEM1024.seedSizeInBytes
                ) { seedPtr in
                    withUnsafeTemporaryAllocation(
                        of: UInt8.self,
                        capacity: MLKEM1024.PublicKey.bytesCount
                    ) { publicKeyPtr in
                        CCryptoBoringSSL_MLKEM1024_generate_key(publicKeyPtr.baseAddress, seedPtr.baseAddress, &self.key)

                        return Data(bytes: seedPtr.baseAddress!, count: MLKEM1024.seedSizeInBytes)
                    }
                }
            }
            ...
            init(seed: some DataProtocol) throws {
                guard seed.count == MLKEM1024.seedSizeInBytes else {
            ...
            func decapsulate(_ encapsulated: Data) throws -> SymmetricKey {
                guard encapsulated.count == MLKEM1024.ciphertextSizeInBytes else {
                    throw CryptoKitError.incorrectParameterSize
                }

                let output = try [UInt8](
                    unsafeUninitializedCapacity: MLKEM1024.sharedSecretSizeInBytes
                ) { bufferPtr, length in
                    ...
                    length = MLKEM1024.sharedSecretSizeInBytes
                }
...
extension MLKEM1024 {
    ...
    public struct PublicKey: Sendable, KEMPublicKey {
        ...
        fileprivate final class Backing {
            ...
            /// - Throws: `CryptoKitError.incorrectKeySize` if the raw representation is not the correct size.
            init(rawRepresentation: some DataProtocol) throws {
                guard rawRepresentation.count == MLKEM1024.PublicKey.bytesCount else {
                    throw CryptoKitError.incorrectKeySize
                }
            ...
            var rawRepresentation: Data {
                ...
                CCryptoBoringSSL_CBB_init(&cbb, MLKEM1024.PublicKey.Backing.bytesCount)
            ...
            func encapsulate() -> KEM.EncapsulationResult {
                withUnsafeTemporaryAllocation(
                    of: UInt8.self,
                    capacity: MLKEM1024.ciphertextSizeInBytes
                ) { encapsulatedPtr in
                    withUnsafeTemporaryAllocation(
                        of: UInt8.self,
                        capacity: MLKEM1024.sharedSecretSizeInBytes
                    ) { secretPtr in
                        ...
                        return KEM.EncapsulationResult(
                            sharedSecret: SymmetricKey(
                                data: Data(bytes: secretPtr.baseAddress!, count: MLKEM1024.sharedSecretSizeInBytes)
                            ),
                            encapsulated: Data(
                                bytes: encapsulatedPtr.baseAddress!,
                                count: MLKEM1024.ciphertextSizeInBytes
                            )
                        )
                    }
                }
...
extension MLKEM1024 {
    ...
    private static let ciphertextSizeInBytes = 1568
}
