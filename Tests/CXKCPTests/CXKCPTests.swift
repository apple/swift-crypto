...
final class CXKCPTests: XCTestCase {
    ...
    func testSHA3_256() throws {
        ...
        let result = input.withUnsafeBytes { inputBytes in
            return SHA3_256(&output, inputBytes.bindMemory(to: UInt8.self).baseAddress, inputBytes.count)
        }
    ...
    func testSHA3_224() throws {
        ...
        let result = input.withUnsafeBytes { inputBytes in
            return SHA3_224(&output, inputBytes.bindMemory(to: UInt8.self).baseAddress, inputBytes.count)
        }
    ...
    func testSHA3_384() throws {
        ...
        let result = input.withUnsafeBytes { inputBytes in
            return SHA3_384(&output, inputBytes.bindMemory(to: UInt8.self).baseAddress, inputBytes.count)
        }
        ...
        let expectedHex = "ec01498288516fc926459f58e2c6ad8df9b473cb0fc08c2596da7cf0e49be4b298d88cea927ac7f539f1edf228376d25"
        let actualHex = output.map { String(format: "%02x", $0) }.joined()
    ...
    func testSHA3_512() throws {
        ...
        let result = input.withUnsafeBytes { inputBytes in
            return SHA3_512(&output, inputBytes.bindMemory(to: UInt8.self).baseAddress, inputBytes.count)
        }
        ...
        let expectedHex = "b751850b1a57168a5693cd924b6b096e08f621827444f70d884f5d0240d2712e10e116e9192af3c91a7ec57647e3934057340b4cf408d5a56592f8274eec53f0"
        let actualHex = output.map { String(format: "%02x", $0) }.joined()
    ...
    func testSHAKE128() throws {
        ...
        let result = input.withUnsafeBytes { inputBytes in
            return SHAKE128(&output, output.count, inputBytes.bindMemory(to: UInt8.self).baseAddress, inputBytes.count)
        }
    ...
    func testSHAKE256() throws {
        ...
        let result = input.withUnsafeBytes { inputBytes in
            return SHAKE256(&output, output.count, inputBytes.bindMemory(to: UInt8.self).baseAddress, inputBytes.count)
        }
