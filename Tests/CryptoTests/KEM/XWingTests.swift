...
final class XWingTests: XCTestCase {
    func testKEM() throws {
        ...
        try XCTAssert(publicKey.rawRepresentation == XWingMLKEM768X25519.PublicKey(dataRepresentation: publicKey.rawRepresentation).rawRepresentation)
        try XCTAssert(privateKey.dataRepresentation == XWingMLKEM768X25519.PrivateKey(bytes: privateKey.dataRepresentation).dataRepresentation)
    ...
    func testXWingMLKEM768X25519TestVectors() throws {
        ...
        for katTest in katTests {
            ...
            let privateKey = try XWingMLKEM768X25519.PrivateKey.generateWithRng(ccrngState: privateKeyDrbg.detRngPtr)
            XCTAssertEqual(privateKey.dataRepresentation, katTest.sk + katTest.pk)
            XCTAssertEqual(privateKey.publicKey.rawRepresentation, katTest.pk)

            let encapDrbg = try SequenceDrbg(katTest.eseed)
            let encapsulatedKey = try privateKey.publicKey.encapsulateWithRng(ccrngState: encapDrbg.detRngPtr)
